...
const mapDispatchToProps = (
  dispatch: (arg0: { type: string; items?: SingleItem[]; y?: number }) => any
) => {
  return {
...
function ItemsList({ itemsList, addItem, setScroll, scrollPos }: any) {
  ...
  const loadMore = useCallback(() => {
    ...
    headers.append("Authorization", "Bearer loremipsum");
    ...
    fetch(`http://127.0.0.1:4010/offers?limit=10&offset=${itemsList.offset}`, {
      ...
      .then(json => {
        const filteredData = json.filter(
          (item: SingleItem) => item.status === "published"
        );
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", shouldLoadMore);
